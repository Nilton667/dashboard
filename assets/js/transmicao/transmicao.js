document.addEventListener('DOMContentLoaded', function(){
  ...
  if(document.getElementById('transmicao-origem')){
    document.getElementById('transmicao-origem').addEventListener('change', function(){
      ...
      if(this.value == 'file'){
        ...
        element.setAttribute('placeholder', 'Youtube vídeos url');
      } else if(this.value == 'link'){
        element.setAttribute('type', 'url');
        element.setAttribute('name', 'video');
        element.setAttribute('placeholder', 'Vídeos url');
      }
    });
  }
  ...
  $(document).delegate('.reproduzir-video', 'click', function(){
    ...
    }else if($(this).attr('data-origem').trim() == 'youtube'){
      var match = atob($(this).attr('data-video').trim()).match(youtReg);
      document.querySelector('#view-video').innerHTML = '<iframe style="width: 100%; border-radius: 8px;" id="youtube-frame" width="100%" height="315" src="https://www.youtube.com/embed/'+match[2]+'" frameborder="0" allow="encrypted-media" allowfullscreen></iframe>';
    } else{
      $.toast({
        heading: 'Alerta',
