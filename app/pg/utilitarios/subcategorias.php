...
<div class="content-header row">
    <div class="content-header-left col-md-4 col-12 mb-2">
        <h3 class="content-header-title mt-2 mb-0">Subsubcategorias</h3>
    </div>
</div>
							...
							<div class="row justify-content-center">
								<div class="col-12 mt-2 p-0">
									...
									if (isset($_GET['filtro']) && $_GET['filtro'] !=''):
										...
										$select = "SELECT * from subcategoria WHERE subcategoria LIKE '%$busca%'";
										$select.= " OR categoria LIKE '%$busca%' ORDER BY id DESC";
									else:
										$select = 'SELECT * from subcategoria ORDER BY id DESC';
									endif;
									...
									if($contar > 0){
										...
										'<div class="custom-control custom-checkbox">
											<input type="checkbox" class="custom-control-input" id="subcategoria-id">
											<label class="custom-control-label" for="subcategoria-id"></label>
										</div>'
										);
										...
										<tr>
                                            ...
                                            <td id="subcategoria-nome-<?php echo $mostra->id; ?>">
                                                <?php echo $mostra->subcategoria;  ?>
                                            </td>
											...
											<th>
												<div class="d-flex w-100 align-items-center">
													<div class="custom-control custom-checkbox d-inline-block">
														...
														subcategoria-select="<?php echo $mostra->id; ?>"
														id="subcategoria-<?php echo $mostra->id; ?>">
														
														<label 
														class="custom-control-label" 
														for="subcategoria-<?php echo $mostra->id; ?>"></label>
													</div>
													
													<i class="las la-edit modal-edit-<?php echo $mostra->id; ?>"
													id="modal-edit"
                                                    data-subcategoria="<?php echo $mostra->subcategoria; ?>"
										...
										$paginacao->queryString = 'subcategorias';
										...
										if (isset($pg) && $pg > 1):
											echo '<script type="text/javascript">location.href = "./?subcategorias";</script>';
											exit(); 
										endif;
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" id="edit-subcategoria-id">
        <div class="form-group">
          <label for="edit-subcategoria-nome">Subcategoria</label>
          <input type="text" class="form-control" id="edit-subcategoria-nome" placeholder="Ex: Software...">
        </div>

        <div>
            <label>Categoria</label>
            <select id="edit-subcategoria-categoria" class="custom-select">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
        <button type="button" class="btn btn-primary" id="edit-subcategoria">Editar</button>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="subcategoria-nome" placeholder="Ex: Software...">
        </div>
        
        <div>
            <label>Categoria</label>
            <select id="subcategoria-categoria" class="custom-select">
                <option value="">-- Selecione uma categoria</option>
                <?php
                $categoria = new Util();
                $categoria = $categoria->getCategoria();
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
        <button type="button" class="btn btn-danger" id="add-subcategoria">Registar</button>
      </div>
    </div>
  </div>
</div>

<script src="assets/js/utilitarios/subcategoria.js"></script>